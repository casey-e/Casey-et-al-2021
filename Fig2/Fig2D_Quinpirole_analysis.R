...
rm(list=ls())
# Load dataframe
df <- read.csv("https://raw.githubusercontent.com//casey-e/Casey-et-al-2022/main/Fig2/Fig2D_Quinpirole_dataframe.csv")
attach(df)
View(df)
...
if (disp[4]<0.05){
  print(paste("dispersion parameter = ",as.character( disp[2] ),", the model has significant overdispersion (p=", as.character(disp[4]), ")"))
}else if (disp[2]<0.7){
  print(paste("dispersion parameter = ",as.character( disp[2] ),", the model has subdispersion"))
}else{
  print(paste("dispersion parameter = ",as.character( disp[2] ),", the model doesn't have significant overdispersion (p=", as.character(disp[4]), ")"))
}

#Evaluate significance with likelihood ratio test (LRT):

#Make netled models:
null_model<- glmer(Number_of_cFOS ~ (1|MouseId), data=df, family=poisson)
neuron_type_model <- glmer(Number_of_cFOS ~ Type_of_neuron + (1|MouseId), data=df, family=poisson)
additive_model<- glmer(Number_of_cFOS ~ Treatment + Type_of_neuron + (1|MouseId), data=df, family=poisson)

#LRT test
lrt=anova(null_model,neuron_type_model,additive_model, TwoWayPoisson)
print(lrt)
...
library(lsmeans)
lsm<-lsmeans(TwoWayPoisson,pairwise ~ Treatment*Type_of_neuron) ## Tukey 
print(lsm$contrasts)
